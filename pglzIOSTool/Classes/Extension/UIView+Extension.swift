...
extension UIView {
    ...
    func borderColor(_ color: UIColor, _ width: CGFloat = 1) {
        layer.borderColor = color.cgColor
        layer.borderWidth = width
    }

    static func createColorView(_ frame: CGRect, _ color: UIColor) -> UIView {
    ...
    ///渐变色
    func gradientLayerColor(bounds: CGRect,leftColor: UIColor, rightColor: UIColor, corner: CGFloat) -> CAGradientLayer {
        let temp = CAGradientLayer()
        temp.frame = bounds
        temp.colors = [leftColor.cgColor,rightColor.cgColor]
        temp.locations = [0.0, 1.0]
        temp.startPoint = CGPoint(x: 0, y: 0)
        temp.endPoint = CGPoint(x: 1, y: 0)
        temp.cornerRadius = corner
        self.layer.insertSublayer(temp, at: 0)
        return temp
    }
    
    func gradientLayerColor(bounds: CGRect,topColor: UIColor, bottomColor: UIColor, corner: CGFloat) -> CAGradientLayer {
        let temp = CAGradientLayer()
        temp.frame = bounds
        temp.colors = [topColor.cgColor,bottomColor.cgColor]
        temp.locations = [0.0, 1.0]
        temp.startPoint = CGPoint(x: 0.5, y: 0)
        temp.endPoint = CGPoint(x: 0.5, y: 1)
        temp.cornerRadius = corner
        self.layer.insertSublayer(temp, at: 0)
        return temp
    }
...
public extension UIView {
    ...
    func addShadow(ofColor color: UIColor = UIColor(red: 253/255.0, green: 120/255.0, blue: 11/255.0, alpha: 0.3), radius: CGFloat = 10, offset: CGSize = CGSize(width: 0, height: 5), opacity: Float = 1.0) {
